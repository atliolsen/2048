...
//	}
	...
	private boolean move(int row, int col, int horizontalDirection, int verticalDirection, Direction dir) {
		...
		while (move) {
			...
			if (board[newRow][newCol] == null) {
				...
				//				board[newRow][newCol].setCanAnimation(true);
				// add to score
			} else {
				move = false;
			}
	...
	private void checkDead() {
		for (int row = 0; row < ROWS; row++) {
			for (int col = 0; col < COLS; col++) {
				...
				if (checkSurroindingTiles(row, col, board[row][col])) {
					return;
				}
			}
		}
	...
	private boolean checkSurroindingTiles(int row, int col, Tile current) {
		if (row > 0) {
